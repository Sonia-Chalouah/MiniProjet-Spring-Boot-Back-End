...
*/
 ...
 public Foyer ajouterFoyerEtAffecterAUniversite(@RequestBody Foyer foyer,
  ...
  @GetMapping("/search/byBloc")
    ...
    public ResponseEntity<List<Bloc>> getBlocsByFoyeid(@PathVariable Long idFoyer) {
        List<Bloc> blocs = iFoyerService.getBlocsByFoyeid(idFoyer);
        return ResponseEntity.ok(blocs);
    }
    @GetMapping("/universite/{universiteId}")
    public ResponseEntity<Foyer> getFoyerByUniversiteId(@PathVariable long universiteId) {
        Foyer foyer = iFoyerService.getFoyerByUniversiteId(universiteId);
        if (foyer != null) {
