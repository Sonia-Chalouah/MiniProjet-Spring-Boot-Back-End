...
*/
 ...
 /*   @Override
    ...
    @Override
    public List<Bloc> getBlocsByFoyeid(Long idFoyer) {
        Foyer foyer = foyerRepository.findById(idFoyer).orElse(null);
        if (foyer != null) {
            return new ArrayList<>(foyer.getBloc());
        }
    ...
    public Map<String, long[]> getFoyerStatistics() {
        ...
        for (Foyer foyer : foyers) {
            ...
            int numberOfBlocs = blocs.size();
            long capacity = foyer.getCapaciteFoyer();

            String statisticKey = "Foyer " + foyer.getIdFoyer();
            long[] statisticValue = new long[] { numberOfBlocs, capacity };
    ...
    @Override
    public Foyer getFoyerByUniversiteId(long universiteId) {
        Universite universite = universiteRepository.findById(universiteId).orElse(null);
        if (universite != null) {
            return universite.getFoyer();
        }
        return null;
    }
